...
export const Route = createFileRoute("/hvala")({
  head: () => ({
    meta: [{ title: "Hvala, upit je poslan" }, { name: "robots", content: "noindex, nofollow" }],
  }),
  component: Hvala,
});
...
function Hvala() {
  const prepare = [
    "Kratak opis organizacije i vaše uloge",
    "Što danas najviše koči rad tima",
    "Postojeći alati (CRM, sustavi, tablice)",
    "Veličina tima i ključni dionici",
    "Preferirani jezik (hrvatski ili engleski)",
  ];
  return (
    <SiteLayout>
      <section className="py-24 md:py-40 bg-[color:var(--bone)] min-h-[80vh]">
        <div className="container-wide">
            ...
            <div className="md:col-span-8">
              <Reveal>
                <h1 className="text-5xl md:text-8xl tracking-[-0.045em] font-medium leading-[0.96]">
                  Hvala.{" "}
                  <span className="text-[color:var(--brand)] font-medium">Upit je poslan.</span>
                </h1>
              </Reveal>
              <Reveal delay={0.1}>
                <p className="mt-10 text-xl md:text-2xl leading-relaxed font-light text-[color:var(--ink)]/85 max-w-2xl">
                  Pregledat ćemo poruku i javiti se s nekoliko kratkih pitanja ili prijedlogom za
                  uvodni razgovor.
                </p>
              </Reveal>
              <Reveal delay={0.2}>
            ...
            <div className="md:col-span-4">
              ...
              <ul className="border-t border-[color:var(--line)]">
                {prepare.map((p, i) => (
                  <li
                    key={p}
                    className="grid grid-cols-[auto_1fr] gap-4 py-3 border-b border-[color:var(--line)]"
                  >
          ...
          <div className="mt-20 pt-8 border-t border-[color:var(--line)] flex flex-wrap items-center gap-6">
            <span className="eyebrow text-[color:var(--muted-text)]">Ako je hitno</span>
            <a
              href={telHref()}
